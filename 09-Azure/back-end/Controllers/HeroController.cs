...
namespace tour_of_heroes_api.Controllers
{
    ...
    public class HeroController(IHeroRepository heroRepository,IConfiguration configuration) : ControllerBase
    {
        ...
        public async Task<IActionResult> DeleteHero(int id)
        {
            ...
            await queueClient.SendMessageAsync(JsonSerializer.Serialize(deleteTask));
        ...
        public IActionResult PeekQueue()
        {
            ...
            var queueClient = new QueueClient(azureConnectionString, "pics-to-delete");

            queueClient.CreateIfNotExists();

            var peeked = queueClient.PeekMessages(10);
            var messages = peeked.Value.Select(m => m.Body.ToString()).ToList();

            Console.WriteLine($"Correct connection. Messages in queue: {messages.Count}");
